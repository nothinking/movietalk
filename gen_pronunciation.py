...
중요 규칙:
1. 각 INDEX의 TEXT에 대해서만 발음을 생성하세요. 절대 인접한 줄과 합치지 마세요.
2. pronunciation은 반드시 100% 한글로만 작성 (영어 단어 금지)
3. 원어민의 실제 빠른 발음을 반영 (gonna, wanna, gotta 등 축약)
4. 모음 사이 t→ㄹ, 연음, 약한 음절 축약 반영
5. notes에는 1~3개의 주요 발음 포인트 포함
...
[
  {{
    ...
    "translation": "한국어 번역",
    "notes": [{{"word": "영어단어", "actual": "한글발음", "meaning": "설명"}}]
  }}
]"""
...
def validate_batch(batch_result: list, expected_indices: list) -> list:
...
def generate_for_video(video_id: str, batch_size: int = 24):
    ...
    for batch_num in range(total_batches):
        ...
        validated = validate_batch(batch_result, expected_indices)
    ...
    if failed_indices:
        ...
        for idx in failed_indices:
            ...
            if result:
                validated = validate_batch(result, [idx])
                if validated:
  ...
  python gen_pronunciation.py VIDEO_ID --batch-size 12  # 배치 크기 조절
    ...
    if args.all:
        ...
        for v in targets:
            ...
            generate_for_video(v['id'], args.batch_size)
            print()
    elif args.video_id:
        print(f"🎬 발음 데이터 생성: {args.video_id}")
        generate_for_video(args.video_id, args.batch_size)
    else:
        parser.print_help()
